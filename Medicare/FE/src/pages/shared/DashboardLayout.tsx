...
const INITIAL_ADMIN_NOTIFICATIONS: AdminNotificationItem[] = [
  {
    ...
    description: 'Có 3 lịch hẹn đang chờ quản trị viên kiểm tra.',
    time: '5 phút trước',
    read: false,
  ...
  },
  {
    ...
    description: 'Một số vật tư trong báo cáo đang ở trạng thái thấp.',
    time: '20 phút trước',
    read: false,
    actionLabel: 'Xem báo cáo vật tư',
    actionPath: '/admin/bao-cao',
  },
  {
    id: 'admin-notif-3',
    title: 'Báo cáo hệ thống đã sẵn sàng',
    description: 'Dữ liệu thống kê mới nhất đã được cập nhật.',
    time: '1 giờ trước',
    read: false,
  ...
  },
  {
    ...
    description: 'Có 2 phản hồi mới từ bệnh nhân đang chờ quản trị viên xem xét.',
    time: '2 giờ trước',
    read: false,
  ...
  {
    id: 'admin-notif-5',
    title: 'Có hoạt động bảo mật bất thường',
    description: 'Hệ thống ghi nhận nhiều lần đăng nhập thử trong nhật ký.',
    time: '3 giờ trước',
    read: false,
  ...
  {
    id: 'admin-notif-6',
    title: 'Phòng khám cần bảo trì',
    description: 'Phòng P-204 được đánh dấu cần kiểm tra thiết bị trước ca chiều.',
    time: 'Hôm qua',
    read: true,
    actionLabel: 'Mở quản lý phòng',
    actionPath: '/admin/phong',
  },
];
...
const DashboardLayout: React.FC<DashboardLayoutProps> = ({ role }) => {
  ...
  const [adminNotifications, setAdminNotifications] = useState<AdminNotificationItem[]>(INITIAL_ADMIN_NOTIFICATIONS);
  ...
  const activeAdminNotificationCount = adminNotifications.filter((notification) => !notification.read).length;
  const selectedNotification = adminNotifications.find((notification) => notification.id === selectedNotificationId);
  ...
  const openAdminNotification = (id: string) => {
  ...
  const goToAdminNotificationTarget = (notification: AdminNotificationItem) => {
            ...
            <div className="flex items-center gap-2 ml-auto">
              {isPatient ? (
                ...
                <div className="relative">
                  ...
                  >
                    <Bell size={18} />
                    {activeAdminNotificationCount > 0 && (
                      <span className="absolute top-1 right-1 min-w-[18px] h-[18px] px-1 bg-red-400 text-white text-[10px] font-semibold rounded-full flex items-center justify-center">
                        {activeAdminNotificationCount > 9 ? '9+' : activeAdminNotificationCount}
                      </span>
                    )}
                  </button>
                  ...
                  {notificationOpen && (
                    <>
                      ...
                      />
                      <div className="absolute right-0 mt-2 w-80 max-w-[calc(100vw-2rem)] rounded-2xl border border-gray-100 bg-white py-2 shadow-xl z-50 dark:bg-slate-800 dark:border-slate-700">
                        <div className="flex items-center justify-between gap-3 px-4 py-3 border-b border-gray-100 dark:border-slate-700">
                          <div>
                            ...
                            <p className="text-xs text-gray-500 dark:text-slate-400">
                              {activeAdminNotificationCount > 0
                                ? `${activeAdminNotificationCount} thông báo chưa đọc`
                          ...
                          {adminNotifications.length === 0 ? (
                            ...
                            adminNotifications.map((item) => (
                              ...
                              >
                                <button
                                  type="button"
                                  onClick={() => openAdminNotification(item.id)}
                                  className="flex flex-1 gap-3 text-left"
                                >
                                  <span
                                    className={`mt-1 h-2.5 w-2.5 shrink-0 rounded-full ${
                                      item.read ? 'bg-gray-300' : 'bg-red-400'
                                    }`}
                                  />
                                  <div className="min-w-0">
                                ...
                                <button
                                  type="button"
                                  onClick={() => deleteAdminNotification(item.id)}
                        ...
                        {selectedNotification && (
                          <div className="border-t border-gray-100 px-4 py-3 dark:border-slate-700">
                            ...
                            <div className="mt-3 flex items-center justify-between gap-2">
                              <button
                                type="button"
                                onClick={() => goToAdminNotificationTarget(selectedNotification)}
                              ...
                              <button
                                type="button"
                                onClick={() => deleteAdminNotification(selectedNotification.id)}
